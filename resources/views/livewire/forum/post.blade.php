<div>
    <div class="mb-6">
        <a href="{{ route('forums.category', $post->forumCategory->slug) }}" wire:navigate class="text-sm text-gym-400 hover:text-orange-400 transition-colors">
    ...
    <div class="space-y-4">
        @foreach($comments as $comment)
            <livewire:components.comment-section :comment="$comment" wire:key="comment-{{ $comment->id }}" />
        @endforeach
    </div>
</div>
